...
type RequestPlannerOptions = {
  signal?: AbortSignal;
};

const OPENAI_API_URL = process.env.NEXT_PUBLIC_OPENAI_API_URL ?? "https://api.openai.com/v1/responses";
const OPENAI_API_KEY = process.env.NEXT_PUBLIC_OPENAI_API_KEY ?? process.env.OPENAI_API_KEY;
const OPENAI_MODEL = process.env.NEXT_PUBLIC_OPENAI_PLANNER_MODEL ?? "gpt-4.1-mini";
...
const STUDY_PLAN_SCHEMA = {
  ...
  schema: {
    ...
    items: {
      ...
      properties: {
        title: { type: "string", description: "Titel van het studieblok." },
        start: {
          type: "string",
          format: "date-time",
          description: "Starttijd in ISO 8601-formaat.",
        },
        end: {
          type: "string",
          format: "date-time",
          description: "Eindtijd in ISO 8601-formaat.",
        },
        type: {
          type: "string",
          enum: ["lesson", "study", "task", "exam"],
          description: "Eventtype binnen de agenda.",
        },
...
export async function requestPlannerEvents(
  prompt: string,
  options?: RequestPlannerOptions
): Promise<PlannerAPIEvent[]> {
  if (!OPENAI_API_KEY) {
    throw new Error(
      "OpenAI API-sleutel ontbreekt. Stel NEXT_PUBLIC_OPENAI_API_KEY of OPENAI_API_KEY in om de planner te gebruiken."
    );
  }
          ...
          content: [
            {
              type: "text",
              text:
                "Je bent een studieplanner. Retourneer uitsluitend een JSON-array met studie-events. Elk object bevat titel, starttijd, eindtijd, type, beschrijving en optioneel locatie. Gebruik ISO 8601-datatijden in de tijdzone van de gebruiker en plan vóór een eventuele deadline.",
            },
          ],
        },
        {
  ...
  if (!response.ok) {
    ...
    throw new Error(`OpenAI API-fout: ${detail}`);
  }

  const payload = (await response.json()) as OpenAIResponse;
  const text = extractPlannerText(payload);

  if (!text) {
    throw new Error("OpenAI gaf geen planner-output terug.");
  }
  ...
  try {
    ...
    throw new Error("OpenAI stuurde geen geldige JSON-array terug.");
  }

  if (Array.isArray(parsed)) {
    return parsed as PlannerAPIEvent[];
  }

  if (parsed && typeof parsed === "object" && Array.isArray((parsed as { events?: unknown[] }).events)) {
    return (parsed as { events: PlannerAPIEvent[] }).events;
  }

  throw new Error("Planner-output bevatte geen events-array.");
}

function extractPlannerText(payload: OpenAIResponse): string | null {
  if (!payload) {
    return null;
  }
  ...
  if (Array.isArray(payload.output)) {
    for (const item of payload.output) {
      ...
      for (const block of item.content) {
        if (block?.type === "output_text" && typeof block.text === "string" && block.text.trim().length) {
          return block.text;
        }
      }
    }
  ...
  if (Array.isArray(payload.choices)) {
    for (const choice of payload.choices) {
      if (choice?.message?.content && choice.message.content.trim().length) {
        return choice.message.content;
      }

      if (typeof choice?.text === "string" && choice.text.trim().length) {
        return choice.text;
      }
    }
  }
